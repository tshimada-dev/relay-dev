...
各フィールドは空にしないこと。後続のタスク分解で参照できる粒度まで具体化すること。
特に設計境界系のキーは、Phase4 で task contract に投影できるよう、対象モジュール名・責務・許可された依存・禁止依存・副作用の入口/出口・状態の所有者を追跡可能な粒度で書くこと。
`visual_contract` はオブジェクトで、以下の schema を厳守すること。特に `design_sources`、`visual_constraints`、`component_patterns`、`responsive_expectations`、`interaction_guidelines` はすべて配列であり、コンポーネント名や breakpoint 名をキーにした連想オブジェクトを出力しないこと。UI変更がない場合は `mode: not_applicable` とし、他の配列は空でよい。
...
`visual_contract` を current run の視覚設計正本として明示すること。下記 schema をそのまま守ること。
