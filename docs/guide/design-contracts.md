...
`verdict ∈ {go, conditional_go, no_go}` が validator で強制されます。
...
越境していれば `no_go`。reviewer は推測ではなく、artifact 中の構造化エビデンスを根拠にします。
...
Phase5-1 reviewer が `visual_contract` と実装成果を突き合わせます。「acceptance_evidence にあるはずの状態が再現されていない」「`color_tokens` 外の値が直書きされている」などを `no_go` として検出します。
