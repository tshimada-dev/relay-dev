...
relay-dev は AI が canonical state を読み書きせずに安全に運用できるよう、**役割を分割した同梱 skill** を `skills/` 配下に持っています（運用記録用の `worklog`、動作確認用の `relay-dev-dummy-run` を含む）。本書では各 skill の責任範囲・ハンドオフ規約・置き場所を説明します。
...
| `relay-dev-dummy-run` | 動作確認 | 確認したい phase / scenario | disposable な `runs/<run-id>/`、それっぽい `run-state.json` と phase artifacts |
...
フルパイプラインを走らせずに、特定 phase / UI / recovery / task-group の動作確認に使える disposable run を生成します。`run-state.json` と必要な phase artifact をまとめて作るため、`show`、lane summary、artifact validator、Phase6 reject recovery などを素早く確認できます。

### 振る舞い

- `skills/relay-dev-dummy-run/scripts/new-relay-dev-dummy-run.ps1` で `runs/<run-id>/` を作る
- `phase5-ready` / `phase6-reject` / `task-group` / `ready-ui` / `approval` の canned scenario を生成する
- `phase0_context.json`、`phase4_tasks.json`、Phase5〜Phase6 の task artifact など、scenario に必要な artifact を保存する
- 生成結果として `run_id`、`run_state_path`、`dummy-run-manifest.json`、次に実行できる `show` コマンドを返す
...
`-RunId` を指定すると固定名の run を作れます。既存 run と同名の場合は拒否されるため、上書きしたい検証時だけ `-Force` を付けます。

### 重要な考え方

- dummy run は `runs/` 配下の検証用データであり、実作業の証跡として扱わない
- まず canned scenario を使い、足りない場合だけ生成後の artifact を局所的に編集する
- `phase6-reject` は artifact contract まで通る基準データとして保つ
...
| 特定 phase / UI / recovery の確認用runが欲しい | `relay-dev-dummy-run` |
