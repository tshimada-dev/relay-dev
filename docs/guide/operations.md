...
| `resume` | `runs/current-run.json` の run を選び、stale `active_job_id` を整合化 |
...
| `invalid_artifact` で停止 | validator が落ちて repair も通らなかった | `troubleshooter` で原因確認、必要なら `course-corrector` で pivot |
