...
provider 実行の timeout policy は `execution` 配下で管理します。現在の tracked default は「警告 / 強制終了」の 3 段階ではなく、**1 attempt を `execution.restart_after_sec` で打ち切って `execution.max_retries` まで再試行する** 形です。
...
長めの Phase5 / Phase6 実装で provider が途中再起動されてしまう場合は、まず `restart_after_sec` を引き上げます。
